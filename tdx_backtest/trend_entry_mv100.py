...
ENTRY_SCORE_MIN = 80         # 最低入场评分（优化到80分）
...
class TrendEntryBacktester:
    ...
    def __init__(self,
        ...
        # 加载市值过滤列表
        self.mv_filter_set: set = set()
        if mv_filter_path and os.path.exists(mv_filter_path):
            df_mv = pd.read_csv(mv_filter_path, encoding="utf-8-sig")
            if "ts_code" in df_mv.columns:
                self.mv_filter_set = set(df_mv["ts_code"].tolist())
                print(f"[Filter] 市值过滤: {len(self.mv_filter_set)} 只股票")
        
        # 预加载K线数据
        self.kline_dict: Dict[str, pd.DataFrame] = {}
        self._date_idx_map: Dict[str, Dict[str, int]] = {}
        self._load_all_klines()
        ...
        print(f"[Backtest] 区间: {self.start_date} ~ {self.end_date}, "
              f"交易日: {len(self.trade_dates)}, 股票: {len(self.kline_dict)}")
    
    def _load_all_klines(self):
        """加载股票K线（主板+双创，支持市值过滤）"""
        from datetime import datetime, timedelta
        ...
        for path in iter_all_day_files(markets=("SH", "SZ")):
            ...
            if ts_code.startswith(("999", "8", "4")):
                continue
            # 市值过滤
            if self.mv_filter_set and ts_code not in self.mv_filter_set:
                n_skip_mv += 1
                continue
            if max(0, 0) and n_ok >= 0:  # 不限制数量
                pass
        ...
        print(f"[Load] 加载 {n_ok} 只股票, 市值过滤 {n_skip_mv}, 跳过 {n_skip}, 耗时 {elapsed:.1f}s")
    ...
    def run_backtest(self, hold_days: int = 5,
        ...
        for i, td in enumerate(self.trade_dates):
            ...
            if verbose and (i % 20 == 0 or i == len(self.trade_dates) - 1):
                elapsed = time.time() - t0
                eta = elapsed / (i + 1) * (len(self.trade_dates) - i - 1) if i > 0 else 0
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="趋势精准入场策略回测（100亿以上市值）")
    ...
    parser.add_argument("--mv-file", type=str, default="high_mv_stocks.csv", help="市值过滤文件")
    parser.add_argument("--out", type=str, default=None, help="交易记录CSV输出路径")
    args = parser.parse_args()
    
    print("=" * 80)
    print("  趋势精准入场策略回测（100亿以上市值）")
    print("=" * 80)
    print(f"  策略参数:")
    print(f"    市值门槛: 100亿以上")
    ...
    print(f"    最低评分: {ENTRY_SCORE_MIN}")
    print("=" * 80)
    
    bt = TrendEntryBacktester(
        start_date=args.start,
        end_date=args.end,
        mv_filter_path=args.mv_file,
    )
    
    res = bt.run_backtest(hold_days=args.hold, verbose=True)
    ...
    if res.get("trade_records"):
        ...
        out_path = args.out or os.path.join(
            os.path.dirname(__file__), "trend_entry_trades_mv100.csv")
