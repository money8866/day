...
pushd solo
python -X utf8 -m sli.update_cache --date %TDATE%
if errorlevel 1 (
    echo [WARN] SLI 缓存增量更新失败，IGE 将沿用最近可用缓存
) else (
    echo [OK] 行情缓存已更新到最近交易日
)
python -X utf8 -m ige.main --date %TDATE% --top 0 > ige\output\ige_run_%TDATE%.log 2>&1
if errorlevel 1 (
    echo [WARN] IGE 运行失败，详见 ige\output\ige_run_%TDATE%.log
) else (
