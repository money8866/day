...
class RealtimeMonitor:
    ...
    def load_watch_pool(self):
        ...
        stocks = []
        # Handle new format: dragons is a list of stock dicts
        dragons_raw = data.get("dragons", [])
        if isinstance(dragons_raw, dict):
            dragons_raw = dragons_raw.get("list", dragons_raw.get("stocks", []))
        if not isinstance(dragons_raw, list):
            dragons_raw = []
        for s in dragons_raw[:10]:
            if not isinstance(s, dict):
                continue
            code = s.get("code", "")
            # pytdx: 0=沪市(主板), 1=深市(含创业板) + 科创板(688xxx)
            # 沪市主板: 600xxx, 601xxx, 603xxx, 605xxx → market=0
            # 科创板688xxx + 深市000/001/002/300/301 → market=1
            market = 0 if (code[0] == '6' and not code.startswith('688')) else 1
            stocks.append({
    ...
    def _get_bars(self, api, code, mkt, count=100):
        """获取1min K线，自动尝试两个market"""
        for m in [mkt, 1 if mkt == 0 else 0]:
            try:
                bars = api.get_security_bars(1, m, code, 0, count)
    ...
    def init_price_cache(self):
        ...
        for stock in self.watch_stocks:
            code = stock["code"]
            mkt = stock["market"]
            bars, used_mkt = self._get_bars(api, code, mkt, 100)
            if bars:
                df = api.to_df(bars)
                ...
                # 获取日线用于量能基准
                daily, _ = self._get_bars(api, code, used_mkt, 5)
                if daily:
                ...
                print(f"  [INIT] {stock['name']}({code}): {len(closes)} bars(mkt={used_mkt})")
            else:
                print(f"  [WARN] {stock['name']}({code}): no bars")
        print(f"[INIT] Cache ready for {len(self.price_cache)} stocks")

    def fetch_realtime_snapshot(self):
        """获取实时快照(盘中)，自动尝试两个market"""
        api = self.server.get_api()
        if not api:
            return {}
        quotes = {}
        for stock in self.watch_stocks:
            ...
            for m in [mkt, 1 if mkt == 0 else 0]:
                try:
                    data = api.get_security_quotes([(m, code)])
    ...
    def fetch_latest_bars(self):
        ...
        for stock in self.watch_stocks:
            code = stock["code"]
            mkt = stock["market"]
            bars, _ = self._get_bars(api, code, mkt, 2)
            if bars:
                df = api.to_df(bars)
