    ...
    def notify_batch(self, warning_list):
        ...
        if self.config.get('notification.enabled', False):
            ...
            if method == 'server':
                ...
                message_parts = [f"【{len(warning_list)}只股票出现信号】"]
                ...
                for i, warning_info in enumerate(warning_list, 1):
                    ...
                    warning_type = warning_info.get('type', '')
                    
                    message_parts.append(
                        f"{i}. {name}({code}) 价格:{price:.2f} 信号:{warning_type}"
                    )
                
                message = "\n".join(message_parts)
                self._send_server_message(title, message.replace("\n", "\n\n"))
    ...
    def _format_stock_warning(self, warning_info):
        ...
        if warning_type == "阳包阴":
            ...
            return (
                ...
                f"前一交易日: 开{details.get('prev_open', 0):.2f} 收{details.get('prev_close', 0):.2f}\n"
                f"今日: 开{details.get('curr_open', 0):.2f} 收{details.get('curr_close', 0):.2f}\n"
                f"时间: {warning_info['time']}"
            )
        elif warning_type == "日线创新高":
            details = warning_info.get('details', {})
            return (
                f"【个股预警 - 日线创新高】\n"
                f"股票: {warning_info['name']} ({warning_info['code']})\n"
                f"当前价格: {warning_info['price']:.2f}\n"
                f"新高价格: {details.get('high', 0):.2f}\n"
                f"周期: {details.get('period', 60)}日\n"
                f"时间: {warning_info['time']}"
            )
        else:
            return (
