# -*- coding: utf-8 -*-
"""
Wave2 深度回调算法 - 二波低吸回测 (向量化加速版)

算法来源: d:/mystock/solo/multi_factor_picker/wave2_pattern_scanner.py (detect_deep_pullback_pattern)
接入框架: tdx_backtest

形态定义 (深度回调):
  - 主板+双创 (排除北交所/指数)
  - 近 SURGE_DAYS=20 天内存在一波拉升 >=20%
  - wave1 高点之后: 回调 >=20% 且 调整天数 >=10 天
  - 排除放量回调: 当 vol_ratio_adj > 1.2 且 回调[10%,20%) 时跳过
  - 排除V型急跌: 当 adjust_days <= 10 且 回调 >=15% 时跳过
...
# =========================================================
# 算法常量 (深度回调 - 来自 wave2_pattern_scanner.py)
# =========================================================
...
# 深度回调核心参数
DEEP_PULLBACK_MIN = 0.20    # 回调下限 20%
DEEP_ADJUST_MIN = 15        # 调整天数下限 15 天

# 排除放量回调阈值
VOL_RATIO_ADJ_MIN = 1.2     # 放量回调量比阈值

# 调整期量比上限 (新增, 缩量深度回调)
VOL_RATIO_ADJ_MAX = 1.0     # 调整期量比上限 (仅考虑缩量/平量)

# 排除V型急跌阈值
VSHAPE_ADJUST_MAX = 10      # V型调整天数上限
VSHAPE_PULLBACK_MIN = 0.15  # V型回调下限

# v3.10 100%胜率硬过滤: 一波涨幅 >= 30%
SURGE_GAIN_ENTRY_MIN = 0.30
...
CHUANGCHUANG_ONLY = True    # True=仅双创 (创业板+科创板)
...
def detect_signals_vectorized(df: pd.DataFrame) -> Tuple[np.ndarray, List[Dict]]:
    """一次性为整只股票的全部交易日计算 深度回调 信号

    核心逻辑:
      1. 滑动窗口找 wave1 (20日内低点->高点涨幅>=20%)
      2. wave1 高点之后: 回调 >=20% 且 调整天数 >=10 天
      3. 排除放量回调: vol_ratio_adj > 1.2 且 回调[10%,20%)
      4. 排除V型急跌: adjust_days <= 10 且 回调 >=15%
      5. 创新低检测: adj_low > pre_low (不创新低)
      6. wave1 高点必须是局部最高点 (前后3天最高)
      7. 入场日硬过滤: 一波涨幅 >= 30%
    """
    n = len(df)
    ...
    for i in range(80, n):
        ...
        # 从 wave1 高点到当前日期的价格序列
        post_closes = C[wave1_high_idx: i + 1]
        if len(post_closes) < 2:
            continue

        # 找到调整期最低点位置
        low_pos = int(np.argmin(post_closes))
        ...
        pullback_pct = (wave1_high_price - C[entry_idx]) / wave1_high_price if wave1_high_price > 0 else 0
        adjust_days = low_pos

        # 深度回调: 回调 >=20% 且 调整 >=10 天
        if not (pullback_pct >= DEEP_PULLBACK_MIN and adjust_days >= DEEP_ADJUST_MIN):
            continue

        # vol_ratio_adj 计算: base_vol = wave1_high_idx前60日均量, adj_vol = wave1_high_idx+1到entry_idx的均量
        vol_base_start = max(0, wave1_high_idx - 60)
        base_vol = VOL[vol_base_start:wave1_high_idx].mean() if wave1_high_idx > 0 else VOL[:i+1].mean()
        if entry_idx > wave1_high_idx:
            adj_vol = VOL[wave1_high_idx + 1: entry_idx + 1].mean()
        else:
            adj_vol = 1.0
        vol_ratio_adj = adj_vol / base_vol if base_vol > 0 else 1.0

        # 排除放量回调: vol_ratio_adj > 1.2 且 回调[10%,20%)
        if vol_ratio_adj > VOL_RATIO_ADJ_MIN and 0.10 <= pullback_pct < 0.20:
            continue

        # 调整期量比上限过滤 (缩量深度回调)
        if vol_ratio_adj >= VOL_RATIO_ADJ_MAX:
            continue

        # 排除V型急跌: adjust_days <= 10 且 回调 >=15%
        if adjust_days <= VSHAPE_ADJUST_MAX and pullback_pct >= VSHAPE_PULLBACK_MIN:
            continue
        ...
        if REQUIRE_HIGHER_LOW:
            ...
            adj_low = C[wave1_high_idx: entry_idx + 1].min()
            if adj_low <= pre_low:
                continue

        # v3.10 100%胜率硬过滤: 一波涨幅 >= 30%
        if wave1_gain < SURGE_GAIN_ENTRY_MIN:
            continue
...
# 回测引擎
# =========================================================
class Wave2DeepPullbackBacktester:
    """Wave2 深度回调回测 (向量化版)"""
    ...
    def run_backtest(self, hold_days: int = 5,
        ...
        for i, td in enumerate(self.trade_dates):
            ...
            if top_n and len(selected) > top_n:
                # 按回调深度排序 (回调越深越优先)
                selected.sort(key=lambda x: -x[1].get("pullback_pct", 0))
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Wave2 深度回调算法回测 (向量化)")
    ...
    print("=" * 80)
    print("  Wave2 深度回调算法回测 (主板+双创, T+1 开盘买入, 向量化)")
    print("=" * 80)
    print(f"  算法参数:")
    print(f"    一波拉升窗口: {SURGE_DAYS} 天, 最低涨幅: {SURGE_MIN*100:.0f}%")
    print(f"    深度回调: 回调 >= {DEEP_PULLBACK_MIN*100:.0f}%, 调整 >= {DEEP_ADJUST_MIN} 天")
    print(f"    排除放量回调: vol_ratio_adj > {VOL_RATIO_ADJ_MIN} 且 回调[{10:.0f}%,{20:.0f}%)")
    print(f"    排除V型急跌: 调整 <= {VSHAPE_ADJUST_MAX} 天 且 回调 >= {VSHAPE_PULLBACK_MIN*100:.0f}%")
    print(f"    创新低检测: {'启用 (adj_low > pre_low)' if REQUIRE_HIGHER_LOW else '禁用'}")
    print(f"    局部最高验证: {'启用 (前后3天)' if REQUIRE_LOCAL_PEAK else '禁用'}")
    print(f"    入场硬过滤: 一波涨幅 >= {SURGE_GAIN_ENTRY_MIN*100:.0f}% (v3.10)")
    print(f"    一波涨幅上限: {WAVE1_GAIN_MAX*100:.0f}% (过滤高位股)")
    print(f"    涨停板开盘跳过 (避免追高)")
    print(f"  股池文件: {args.pool}")
    print(f"  板块范围: 主板+双创全市场")
    print("=" * 80, flush=True)

    bt = Wave2DeepPullbackBacktester(
    ...
    if res.get("trade_records"):
        ...
        print("\n  调整天数分档胜率:")
        for lo, hi in [(10, 15), (15, 20), (20, 30), (30, 999)]:
            sub = [r["return"] for r in recs if lo <= r["adjust_days"] < hi]
            if sub:
                wr = sum(1 for x in sub if x > 0) / len(sub) * 100
                avg = np.mean(sub)
                label = f"调整{lo}-{hi}天" if hi < 999 else f"调整{lo}天+"
                print(f"    {label}: {len(sub)}笔, 胜率{wr:.1f}%, 均收益{avg:.2f}%")

        print("\n  回调深度分档胜率:")
        for lo, hi in [(20, 25), (25, 30), (30, 40), (40, 999)]:
            sub = [r["return"] for r in recs if lo <= r["pullback_pct"] < hi]
            if sub:
                wr = sum(1 for x in sub if x > 0) / len(sub) * 100
                avg = np.mean(sub)
                label = f"回调{lo}-{hi}%" if hi < 999 else f"回调{lo}%+"
                print(f"    {label}: {len(sub)}笔, 胜率{wr:.1f}%, 均收益{avg:.2f}%")

        print("\n  一波涨幅分档胜率:")
        for lo, hi in [(30, 40), (40, 50), (50, 65), (65, 80)]:
        ...
        print("\n  调整期量比分档胜率:")
        for lo, hi in [(0, 0.5), (0.5, 0.8), (0.8, 1.0), (1.0, 1.2), (1.2, 999)]:
    ...
    if args.out:
        ...
        out_path = r"d:\mystock\solo\tdx_backtest_wave2_deep_pullback_trades.csv"
