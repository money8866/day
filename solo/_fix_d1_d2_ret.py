...
print('\n获取交易日历...')
try:
    trade_cal = pro.trade_cal(exchange='SSE', start_date='20260101', end_date='20260630')
    trade_dates = set(trade_cal[trade_cal['is_open'] == 1]['cal_date'].astype(str))
    print(f'交易日历: {len(trade_dates)} 天')
except Exception as e:
    print(f'获取交易日历失败: {e}')
    trade_dates = None
    ...
    try:
        ...
        for _, row in group.iterrows():
            ...
            results.append({
                ...
                'sig_close': sig_close,
                'd1_date': d1_date,
                'd1_close': d1_close if d1_ret is not None else None,
                'd1_ret': d1_ret,
                'd2_date': d2_date,
                'd2_close': d2_close if d2_ret is not None else None,
                'd2_ret': d2_ret,
            })
