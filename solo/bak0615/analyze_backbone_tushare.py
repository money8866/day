...
def get_concept_map():
    ...
    cache_file = os.path.join(CACHE_DIR, "tdx_concept_members.pkl")
    ...
    try:
        print("   正在获取通达信概念板块列表...")
        index_df = pro.tdx_index()
        
        if index_df is None or index_df.empty:
            print("   获取板块列表失败")
            return {}, {}
        
        print(f"   获取到 {len(index_df)} 个通达信板块")
        ...
        for idx, row in index_df.iterrows():
            ...
            try:
                member_df = pro.tdx_member(ts_code=ts_code)
...
def build_maps_from_df(df):
    ...
    print(f"   找到 {len(concept_map)} 个有效概念板块（成分股>=30）")
...
def get_stock_name_map():
    try:
        cache_file = os.path.join(CACHE_DIR, "tdx_concept_detail.pkl")
        if os.path.exists(cache_file):
            df = pd.read_pickle(cache_file)
            name_map = {}
