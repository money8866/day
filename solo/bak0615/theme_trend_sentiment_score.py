...
def calc_rotation_cycle(theme_name, current_t_score, current_s_score):
    ...
    核心思想：用排名百分位（0-100%）替代绝对分数进行分析。
    排名越高表示该主题在当天所有主题中趋势越强。
    
    轮动周期全景：
      启动 → 上升中 → 短期爆发/中期持续 → 分歧回调 → 高潮风险 → 退潮回避
      （震荡整理、弱势整理作为中继状态）
    ...
    try:
        ...
        # ========== 多周期平均排名（供输出用） ==========
        avg_rank_60d = round(np.mean(rank_history), 1) if n >= 10 else None
        recent_20 = rank_history[-min(20, n):]
        avg_rank_20d = round(np.mean(recent_20), 1) if len(recent_20) >= 5 else None
        recent_10 = rank_history[-min(10, n):]
        avg_rank_10d = round(np.mean(recent_10), 1) if len(recent_10) >= 5 else None
        ...
        if n >= 3:
            y3 = rank_history[-3:]
            if np.std(y3) > 0:
                rank_slope_3d = round(np.polyfit(np.arange(3), y3, 1)[0], 2)
            else:
                rank_slope_3d = 0.0
        else:
            rank_slope_3d = 0.0
        ...
        if n >= 5:
            y5 = rank_history[-5:]
            if np.std(y5) > 0:
                rank_slope_5d = round(np.polyfit(np.arange(5), y5, 1)[0], 2)
            else:
                rank_slope_5d = 0.0
        else:
            rank_slope_5d = None
        ...
        # 5日排名涨幅（rank版）
        rank_ret_5d = round(rank_history[-1] - rank_history[-6], 2) if n >= 6 else 0

        # 近5天排名波动幅度（最高-最低）
        recent_5_ranks = rank_history[-5:] if n >= 5 else rank_history
        rank_vol_5d = np.max(recent_5_ranks) - np.min(recent_5_ranks)

        # 近5天情绪均值
        s_5 = np.mean(sentiment_history[-5:]) if n >= 5 else current_senti

        # 历史排名高位 + 近期（7天）高位
        rank_high = np.max(rank_history)
        recent_high = np.max(rank_history[-min(7, n):])

        # 当日排名变化（用于判断背离）
        daily_rank_change = rank_history[-1] - rank_history[-2] if n >= 2 else 0
        ...
        # 近10天最低排名
        recent_low_10 = np.min(rank_history[-min(10, n):]) if n >= 5 else rank_early

        # ========== 判断横向整理（连续2日斜率绝对值<0.5） ==========
        stale = False
        if n >= 4:
            y3_prev = rank_history[-4:-1]
            if np.std(y3_prev) > 0:
                prev_slope = np.polyfit(np.arange(3), y3_prev, 1)[0]
            else:
                prev_slope = 0
            if abs(rank_slope_3d) < 0.5 and abs(prev_slope) < 0.5:
                stale = True

        # ========== 边界条件处理 ==========

        # ① 极端低位
        if current_rank_pct == 0:
            return "极端低位警告", f"当前排名为0%分位，所有主题中最弱"

        # ② 极端高位
        if current_rank_pct == 100:
            return "极端高位警告", f"当前排名为100%分位，所有主题中最强"

        # ③ 横向整理（连续横盘）
        if stale:
            pos = "高位" if current_rank_pct >= 65 else ("中位" if current_rank_pct >= 35 else "低位")
            return "横向整理", f"排名{current_rank_pct:.0f}%分位({pos})，连续横盘无方向"

        # ④ 背离：3日斜率>3但当日排名下跌 → 降级为分歧回调
        if rank_slope_3d > 3 and daily_rank_change < -2:
            stage = "分歧回调"
            desc = f"3日斜率{rank_slope_3d:.1f}>3但当日排名回落{daily_rank_change:.1f}%，伪突破特征，降级处理"
            return stage, desc

        # ========== 主判定逻辑 ==========

        # --- 启动 ---
        # 历史排名<30% AND 当前排名≥35% AND 3日斜率>2
        if rank_early < 30 and current_rank_pct >= 35 and rank_slope_3d > 2:
            return "启动", f"排名从底部{rank_early:.0f}%分位回升至{current_rank_pct:.0f}%分位，3日斜率{rank_slope_3d:.1f}>2，底部启动确认"
        if recent_low_10 < 30 and current_rank_pct >= 35 and rank_slope_3d > 2:
            return "启动", f"排名从低位{recent_low_10:.0f}%分位回升至{current_rank_pct:.0f}%分位，3日斜率{rank_slope_3d:.1f}>2，启动信号"

        # --- 短期爆发 ---
        # 3日斜率>4 AND 5日涨幅>8% AND 当前排名≥60%
        if rank_slope_3d > 4 and rank_ret_5d > 8 and current_rank_pct >= 60:
            return "短期爆发", f"3日斜率{rank_slope_3d:.1f}>4，5日排名升{rank_ret_5d:.1f}>8%，排名{current_rank_pct:.0f}%分位，强势爆发"
        # 宽松版：斜率>4 + 排名≥70%
        if rank_slope_3d > 4 and current_rank_pct >= 70 and s_5 >= 45:
            return "短期爆发", f"3日斜率{rank_slope_3d:.1f}>4，排名跃升至{current_rank_pct:.0f}%分位，短期爆发"

        # --- 高潮风险 ---
        # 当前排名≥90% AND 情绪分≥60
        if current_rank_pct >= 90 and s_5 >= 60:
            return "高潮风险", f"排名{current_rank_pct:.0f}%分位≥90%，情绪{s_5:.0f}≥60，亢奋赶顶"
        if avg_rank_10d and avg_rank_10d >= 85 and rank_vol_5d < 4 and current_rank_pct >= 85:
            return "高潮风险", f"近10天平均排名{avg_rank_10d:.0f}%分位，极高位横盘，风险积聚"

        # --- 分歧回调 ---
        # 从近期高位回落10-20个百分点 + 情绪未崩
        drop_from_high = recent_high - current_rank_pct
        if (recent_high > 60 and 10 <= drop_from_high <= 25 and
                s_5 >= 40 and current_rank_pct >= 30):
            return "分歧回调", f"排名从近期高点{recent_high:.0f}%回落{drop_from_high:.1f}个百分点，情绪{s_5:.0f}尚可，正常调整"

        # --- 中期持续 ---
        # 当前排名55%-85% + 5日波幅<7% + 情绪配合
        if (55 <= current_rank_pct <= 85 and rank_vol_5d < 7 and
                s_5 >= 42 and abs(rank_slope_3d) < 4):
            return "中期持续", f"排名{current_rank_pct:.0f}%分位(55%-85%)，5日波幅{rank_vol_5d:.1f}<7%，情绪{s_5:.0f}，趋势延续健康"

        # --- 上升中 ---
        # 3日斜率>2 AND 当前排名35%-75%
        if rank_slope_3d > 2 and 35 <= current_rank_pct < 75:
            return "上升中", f"排名{current_rank_pct:.0f}%分位(35%-75%)，3日斜率{rank_slope_3d:.1f}>2，趋势向上推进"
        # 兜底：斜率>1.5 + 排名≥35%
        if rank_slope_3d > 1.5 and current_rank_pct >= 35:
            return "上升中", f"排名{current_rank_pct:.0f}%分位，斜率{rank_slope_3d:.1f}，趋势向上"

        # --- 退潮回避 ---
        # 排名<30% + 情绪崩溃(<45)
        if current_rank_pct < 30 and s_5 < 45:
            return "退潮回避", f"排名{current_rank_pct:.0f}%分位<30%+情绪{s_5:.0f}<45，资金离场，退潮回避"
        # 从高位持续下滑
        if rank_early > 55 and current_rank_pct < 40 and rank_slope_3d < -1 and s_5 < 45:
            return "退潮回避", f"排名从{rank_early:.0f}%持续下滑至{current_rank_pct:.0f}%+情绪低迷，趋势转弱"

        # --- 震荡整理 ---
        # 排名35%-65% + |3日斜率|<1.5
        if 35 <= current_rank_pct <= 65 and abs(rank_slope_3d) < 1.5:
            return "震荡整理", f"排名{current_rank_pct:.0f}%分位(35%-65%)，斜率{rank_slope_3d:.1f}，方向不明，震荡整理"

        # --- 弱势整理 ---
        # 排名<35% + 情绪尚可
        if current_rank_pct < 35 and s_5 >= 45:
            return "弱势整理", f"排名{current_rank_pct:.0f}%分位<35%，情绪{s_5:.0f}尚可，低位弱震荡"

        # --- 兜底 ---
        return ("震荡整理" if current_rank_pct >= 50 else "弱势整理",
                f"排名{current_rank_pct:.0f}%分位，信号不明确，{'震荡' if current_rank_pct >= 50 else '偏弱'}整理")
...
def analyze_style_trend(results):
    ...
    style_data = defaultdict(lambda: {
        ...
        "tradeable_count": 0, "cycle_dist": defaultdict(int),
    })
    
    tradeable_cycles = {"启动", "上升中", "短期爆发", "中期持续"}
    climax_cycles = {"高潮风险", "退潮回避"}
    ...
    for r in results:
        ...
        d["cycle_dist"][cycle] += 1
        if cycle in tradeable_cycles:
            d["tradeable_count"] += 1
    ...
    for style, d in style_data.items():
        ...
        style_summary.append({
            ...
            "cycle_dist": dict(d["cycle_dist"]), "themes": d["themes"], "status": status,
        })
...
def save_report_text(results):
    ...
    w("\n轮动周期全景:")
    w("-" * 60)
    for cycle in ["启动", "上升中", "短期爆发", "中期持续", "分歧回调", "高潮风险", "退潮回避"]:
    ...
    # ========== 风格维度中期跟踪 ==========
    _, style_text = analyze_style_trend(results)
    w(style_text)
