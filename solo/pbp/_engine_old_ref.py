...
class PBPEngine:
    ...
    def _platform_stats(self, df: pd.DataFrame, s: int, e: int, hi: float, lo: float, atr: float) -> dict:
        """平台内部统计：波动收敛/阻力测试/支撑测试/量能收缩/均线结构"""
        cfg = self.cfg
        highs = df['high'].values.astype(float)
        lows = df['low'].values.astype(float)
        closes = df['close'].values.astype(float)
        ...
        trs = [_tr(i) for i in range(s, e)]
        tr_1h = float(np.mean(trs[:half])) if half > 0 else 0.0
        tr_2h = float(np.mean(trs[half:])) if half > 0 else 0.0
        atr_compress = tr_2h / tr_1h if tr_1h > 0 else 1.0
        amp_1h = float(np.mean([(highs[i] - lows[i]) / closes[i] for i in range(s, s + half)])) if half > 0 else 0.0
        amp_2h = float(np.mean([(highs[i] - lows[i]) / closes[i] for i in range(s + half, e)])) if half > 0 else 0.0
        amp_shrink = amp_2h / amp_1h if amp_1h > 0 else 1.0
        std_1h = float(np.std(closes[s:s + half])) if half > 0 else 0.0
        std_2h = float(np.std(closes[s + half:e])) if half > 0 else 0.0
        ...
        while i < e:
            j = min(i + seg_len, e)
            if float(highs[i:j].max()) >= hi - tol:
                res_tests += 1
            if float(lows[i:j].min()) <= lo + tol:
                sup_tests += 1
            i = j

        # ── 量能收缩：后半段/前半段 ──
        v_1h = float(np.mean(vols[s:s + half])) if half > 0 else 0.0
        v_2h = float(np.mean(vols[s + half:e])) if half > 0 else 0.0
        vol_shrink = v_2h / v_1h if v_1h > 0 else 1.0

        # ── 持续放量大阴线检测（假平台特征）──
        vol_ma20_seg = df['vol_ma20'].values
        big_dn_days = 0
        for i in range(s, e):
            vm = vol_ma20_seg[i]
            if not np.isnan(vm) and vm > 0 and vols[i] > vm * 1.5 and closes[i] < opens[i]:
                big_dn_days += 1
    ...
    def score(self, df: pd.DataFrame, ts_code: str = '', name: str = '',
              ...
              theme_info: Optional[dict] = None) -> PBPResult:
        cfg = self.cfg
        ...
        for b in range(search_start, end_idx + 1):
            platform = self._find_platform(df, b)
        ...
        cur_platform = self._find_platform(df, end_idx)
