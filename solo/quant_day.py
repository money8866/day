...
def run(target_date=None, simple_mode=False):
    ...
    ### 这里加入二波低吸的策略引入###
    # 二波低吸策略：扫描 bull_stocks_qualified.csv 中的股票
    wave2_csv_path = r'D:\mystock\solo\report_daily\bull_stocks_qualified.csv'
    if os.path.exists(wave2_csv_path):
        print(f'\n[二波低吸] 开始扫描 {wave2_csv_path}...')
        try:
            from wave2_pattern_scanner import WavePatternDetector
            wave2_detector = WavePatternDetector(force_date=TRADE_DATE)
            df_wave2_pool = pd.read_csv(wave2_csv_path)
            # CSV 中股票代码列名可能是 ts_code 或 code，且可能不带后缀（需补全）
            if 'ts_code' in df_wave2_pool.columns:
                raw_codes = df_wave2_pool['ts_code'].tolist()
            elif 'code' in df_wave2_pool.columns:
                raw_codes = df_wave2_pool['code'].tolist()
            else:
                raw_codes = []
            # 补全为完整 ts_code 格式（如 2709 → 002709.SZ）
            wave2_codes = []
            for c in raw_codes:
                c = str(c).strip()
                if '.' in c:
                    wave2_codes.append(c)
                else:
                    c_pad = c.zfill(6)
                    if c_pad.startswith(('6', '9')):
                        wave2_codes.append(f'{c_pad}.SH')
                    else:
                        wave2_codes.append(f'{c_pad}.SZ')
            
            if wave2_codes:
                print(f'[二波低吸] 股票池: {len(wave2_codes)} 只')
                wave2_df = wave2_detector.scan_pool(wave2_codes, 'all', today_only=False)
                
                if not wave2_df.empty:
                    print(f'[二波低吸] 命中: {len(wave2_df)} 只')
                    for _, r in wave2_df.iterrows():
                        result_dx.append({
                            '代码': r['ts_code'],
                            '名称': r.get('name', ''),
                            '现价': r.get('close', 0),
                            '涨跌幅': r.get('pct_chg', 0),
                            '成交额': r.get('amount', 0),
                            '总市值（亿元）': r.get('total_mv', 0)/10000 if r.get('total_mv') else 0,
                            'total_market_cap': r.get('total_mv', 0) * 10000 if r.get('total_mv') else 0,
                            'market_cap': r.get('total_mv', 0) * 10000 if r.get('total_mv') else 0,
                        ...
                        print(f'二波低吸命中: {r["ts_code"]} {r.get("name", "")} {r.get("pattern", "")} 评分{r.get("score", 0)}分')
                else:
                    print(f'[二波低吸] 未命中')
            else:
                print(f'[二波低吸] 股票池为空')
        ...
        print(f'[二波低吸] 文件不存在: {wave2_csv_path}')
    ####引入结束 ###
