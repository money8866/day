    ...
    try:
        ...
        if df is not None and len(df) > 0:
            if '占净值比例' in df.columns:
                ...
                # 过滤掉占比小于0.1%的股票，避免权重太低的噪音
                df = df[df['占净值比例'] >= 0.1]
            
            with open(cache_file, 'wb') as f:
                pickle.dump(df, f)
            print(f"  Success, {len(df)} stocks (filtered >0.1%)")
            return df
        else:
...
def etf_score(df, industry, index_df):
    ...
    # 3日涨幅（新增，更敏感）
    if len(df) >= 5:
        pct3 = (df['close'].iloc[-1] / df['close'].iloc[-4] - 1) * 100
        score += pct3 * 2
    ...
    # ========== 第十维度：成交量结构优化 ==========
    score += volume_structure_enhanced(df)
    
    # ========== 第十一维度：赚钱效应（红盘天数） ==========
    score += breadth_score(df) * 1.5  # 提高权重
    
    # ========== 第十二维度：近期涨停基因（新增游资特色指标） ==========
...
def volume_structure_enhanced(df):
    try:
        ...
        # 新增：今日放量评分
        latest = df.iloc[-1]
        vol_ratio = latest['vol'] / latest['vol5'] if latest['vol5'] > 0 else 1
        
        vol_bonus = 0
        if vol_ratio > 2.0:  # 倍量
            vol_bonus = 8
        elif vol_ratio > 1.5:  # 放量50%
            vol_bonus = 5
...
def check_stock_conditions(df):
    ...
    # 成交量放宽（游资喜欢放量，允许倍量）
    if pd.isna(latest['vol_ratio']) or latest['vol_ratio'] < 0.6:
        return False, f"Vol ratio too low {latest.get('vol_ratio', 0):.2f}"
    
    # 20日涨幅过高（不追高）
    if latest.get('return_20', 0) >= 60:
        return False, f"Rise too big {latest.get('return_20', 0):.1f}%"
    
    # ========== 计算个股评分 ==========
    stock_score = calculate_stock_score(df)
    
    return True, {
        'stock_score': stock_score,
        'return_3': round(latest.get('return_3', 0), 2),
        'return_5': round(latest.get('return_5', 0), 2),
...
def calculate_stock_score(df):
    ...
    # 1. 短期涨幅（5日+3日）
    score += latest.get('return_5', 0) * 1.5
    score += latest.get('return_3', 0) * 2
...
def analyze_etf(industry, ts_code, index_df):
    ...
    if qualified:
        # 按个股评分从高到低排序（游资风格）
        qualified_sorted = sorted(qualified, key=lambda x: x['stock_score'], reverse=True)
        
        print(f"\nFound {len(qualified)} qualified stocks:")
        print(f"\n  {'Rank':<4} {'Code':<12} {'Name':<10} {'Score':<8} {'Weight':<8} {'5d Rise':<10} {'Vol Ratio':<10} {'MA5/MA20':<15}")
        print(f"  {'-'*100}")
        
        for i, s in enumerate(qualified_sorted[:15], 1):
            print(f"  {i:<4} {s['ts_code']:<12} {s['name']:<10} {s['stock_score']:<8.1f} {s['weight']:<8} {s['return_5']:<10.2f} {s['vol_ratio']:<10.2f} {s['ma5']:.2f}/{s['ma20']:.2f}")
    else:
...
def main():
    ...
    if all_qualified_stocks:
        # 按个股综合评分排序（游资风格）
        all_qualified_stocks_sorted = sorted(all_qualified_stocks, key=lambda x: x['stock_score'], reverse=True)
        
        print(f"\n\nAll Qualified Stocks (Total: {len(all_qualified_stocks)}) - 按游资评分排序")
        print(f"\n  {'Rank':<4} {'Name':<10} {'Industry':<15} {'Code':<12} {'Score':<8} {'5d Rise':<10} {'Vol Ratio':<10} {'ETF Score':<10}")
        print(f"  {'-'*110}")
        
        for i, s in enumerate(all_qualified_stocks_sorted[:30], 1):
            print(f"  {i:<4} {s['name']:<10} {s['industry']:<15} {s['ts_code']:<12} {s['stock_score']:<8.1f} {s['return_5']:<10.2f} {s['vol_ratio']:<10.2f} {s['etf_score']:<10.2f}")
