...
class TradeConfig:
    ...
    # V5新增：情绪和板块过滤
    MIN_EMOTION_SCORE = 30  # 情绪分最低30分
    TOP_MAIN_SECTORS = 15   # 只在前15个主线板块中选股
...
def is_stabilization_kline_v5(kline, prev_klines):
    try:
        open_p = kline['open']
        close_p = kline['close']
        high_p = kline['high']
...
def analyze_rubbing_stabilization_v5(daily_df, zt_history):
    try:
        ...
        for i in range(len(klines) - 3):
            ...
            if is_rubbing_line_v5(k1, k2):
                if i + 1 < len(klines):
                    next_kline = klines[-(i+1)]
                    prev_klines = klines[:-(i+1)]
                    
                    if is_stabilization_kline_v5(next_kline, prev_klines):
                        score = 50
...
def get_market_emotion_v5(trade_date):
    try:
        original_cwd = os.getcwd()
        os.chdir(MYSTOCK_DIR)
        import emotion
        emotion_score = emotion.analyze_market_emotion(None)
        os.chdir(original_cwd)
        return emotion_score
    except Exception as e:
        print(f"⚠️ 获取大盘情绪失败: {e}")
        return None


def get_main_sectors_v5():
    try:
        original_cwd = os.getcwd()
        os.chdir(MYSTOCK_DIR)
        import block
        hot_sectors = block.analyze_hot_sectors()
        os.chdir(original_cwd)
        
        if hot_sectors is not None and len(hot_sectors) > 0:
            return set(hot_sectors.head(TradeConfig.TOP_MAIN_SECTORS)['主线'].tolist())
        return set()
    except Exception as e:
        print(f"⚠️ 获取主线板块失败: {e}")
        return set()


def filter_stocks_by_trade_signal_v5(trade_date, lookback_days=20):
    try:
        print(f"\n{'='*60}")
        print(f"🔍 V5筛选交易信号股票（大盘情绪+主线板块）")
        print(f"{'='*60}")
        
        # V5第一步：检查大盘情绪
        emotion_info = get_market_emotion_v5(trade_date)
        emotion_score = emotion_info.get('情绪分', 0) if emotion_info else 0
        
        if emotion_score < TradeConfig.MIN_EMOTION_SCORE:
            print(f"⚠️ 大盘情绪分 {emotion_score:.1f} 低于 {TradeConfig.MIN_EMOTION_SCORE}，不操作")
            return []
        else:
            print(f"✅ 大盘情绪: {emotion_score:.1f}分，适合操作")
        
        # V5第二步：获取主线板块
        main_sectors = get_main_sectors_v5()
        if len(main_sectors) > 0:
            print(f"✅ 主线板块 ({len(main_sectors)}): {', '.join(list(main_sectors)[:5])}...")
        else:
            print(f"⚠️ 未识别到主线板块，全市场选股")
        ...
        for row in conn.execute(query, (start_date, trade_date)):
            ...
            # V5第三步：主线板块过滤（如果有）
            if len(main_sectors) > 0 and industry and industry not in main_sectors:
                continue
            ...
            try:
                ...
                is_signal, signal_desc, signal_score = analyze_rubbing_stabilization_v5(daily_df, zt_history)
                ...
                if is_signal and signal_score >= TradeConfig.STABILIZATION_SCORE_MIN:
                    ...
                    signals.append({
                        ...
                        'in_main_sector': industry in main_sectors if industry else False
                    })
                    
                    sector_tag = " [主线]" if industry in main_sectors else ""
                    print(f"✓ V5信号: {name} {signal_desc} | 分数:{signal_score}{sector_tag}")
        ...
        if len(main_sectors) > 0:
            main_sector_signals = [s for s in signals if s.get('in_main_sector', False)]
            other_signals = [s for s in signals if not s.get('in_main_sector', False)]
            signals = main_sector_signals + other_signals
        
        print(f"\n📊 V5信号筛选完成: 共{len(signals)}个信号")
...
def execute_buy_v5(ts_code, name, price, trade_date, signal_score):
    try:
        positions = get_holding_positions_v5()
        ...
        cursor.execute('''
            ...
            VALUES (?, ?, '买入', ?, ?, ?, ?, ?)
        ''', (ts_code, name, price, shares, amount, trade_date, "揉搓线企稳V5买入"))
...
def execute_sell_v5(position_id, ts_code, name, price, reason, trade_date):
    try:
        conn = sqlite3.connect(TRADE_DB)
        cursor = conn.cursor()
...
def check_and_execute_trades_v5(trade_date):
    try:
        positions = get_holding_positions_v5()
        ...
        for pos in positions:
            ...
            if current_price >= target:
                success, result = execute_sell_v5(pos['id'], ts_code, name, current_price, "V5止盈5%", trade_date)
                if success:
                    print(f"✅ V5止盈卖出: {name} | 盈利{result['profit_rate']:.2f}%")
                continue
            
            if current_price <= stop_loss:
                success, result = execute_sell_v5(pos['id'], ts_code, name, current_price, "V5止损3%", trade_date)
                if success:
                    print(f"❌ V5止损卖出: {name} | 亏损{result['profit_rate']:.2f}%")
                continue
            
            if profit_rate > 5:
                peak_profit = pos.get('peak_profit', profit_rate)
                if profit_rate < peak_profit - 2:
                    success, result = execute_sell_v5(pos['id'], ts_code, name, current_price, "V5追踪止盈", trade_date)
                    if success:
                        print(f"✅ V5追踪止盈: {name} | 盈利{result['profit_rate']:.2f}%")
        
    except Exception as e:
        print(f"检查持仓失败: {e}")


def daily_trade_v5(trade_date):
    try:
        print(f"\n{'='*70}")
        print(f"📈 V5每日交易 | {trade_date}")
        print(f"{'='*70}")
        
        init_trade_db()
        
        print(f"\n[1/3] 检查持仓...")
        check_and_execute_trades_v5(trade_date)
        
        print(f"\n[2/3] V5筛选买入信号...")
        signals = filter_stocks_by_trade_signal_v5(trade_date)
        ...
        for signal in signals[:TradeConfig.MAX_POSITIONS - len(positions)]:
            ...
            success, result = execute_buy_v5(ts_code, name, price, trade_date, signal['signal_score'])
            
            if success:
                print(f"✅ V5买入成功: {name} @ {price:.2f}")
            else:
                print(f"⚠️ V5买入失败: {name} - {result}")
        
        print_trade_stats_v5()
        
    except Exception as e:
        print(f"❌ V5交易失败: {e}")


def print_trade_stats_v5():
    try:
        conn = sqlite3.connect(TRADE_DB)
        ...
        if not sell_df.empty:
            ...
            if total > 0:
                ...
                print(f"  V5胜率: {win_rate:.1f}% ({wins}/{total})")
...
def backtest_strategy_v5(start_date, end_date, lookback_days=20):
    try:
        print(f"\n{'='*70}")
        print(f"📈 V5策略回测 | {start_date} ~ {end_date}")
        print(f"{'='*70}")
                            ...
                            if is_rubbing_line_v5(k1, k2):
                                ...
                                for j in range(i+2, len(klines)):
                                    ...
                                    if sell_price >= target_price:
                                        ...
                                        total_trades += 1
                                        win_trades += 1
                                        print(f"✓ V5止盈: {name} {profit_rate*100:.2f}%")
                                        break
                                    elif sell_price <= stop_price:
                                        total_profit += profit_rate
                                        total_trades += 1
                                        print(f"✗ V5止损: {name} {profit_rate*100:.2f}%")
                                        break
        ...
        print(f"\n📊 V5回测结果")
        print(f"  V5总交易: {total_trades}笔")
        print(f"  V5胜率: {win_trades/total_trades*100:.1f}%")
        print(f"  V5总收益率: {total_profit*100:.2f}%")
        if total_trades > 0:
            print(f"  V5平均收益: {total_profit/total_trades*100:.2f}%")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='涨停二波交易系统 V5')
    ...
    if args.mode == 'trade':
        ...
        backtest_strategy_v5(start, end)
