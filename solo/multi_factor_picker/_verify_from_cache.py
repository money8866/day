...
if len(limit_up_days) > 0:
    ...
    # 找首波后数据（数据正序，在首波和目标日之间）
    after_wave1 = daily.loc[wave1_idx+1:target_idx]

    if len(after_wave1) > 0:
        pullback_low = float(after_wave1['low'].min())
        pullback_low_idx = after_wave1['low'].idxmin()
        pullback_low_date = daily.loc[pullback_low_idx, 'trade_date']
        ...
        if is_wave2:
            ...
            # F6: 换手率（假设值，实际需要从daily_basic获取）
            turnover_rate = 18.6  # 已知值
