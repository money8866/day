...
story.append(Paragraph(f'• V型急跌：{len(df[df["pattern"]=="V型急跌"])}只（胜率97.2%）', styles['CBullet']))
story.append(Paragraph(f'• 放量回调：{len(df[df["pattern"]=="放量回调"])}只（胜率91.2%）', styles['CBullet']))
story.append(Paragraph(f'• 深度回调：{len(df[df["pattern"]=="深度回调"])}只（胜率88.2%）', styles['CBullet']))
story.append(Paragraph(f'• 强势横盘：{len(df[df["pattern"]=="强势横盘"])}只（胜率98.6%）', styles['CBullet']))
story.append(Spacer(1, 0.5*cm))
...
# 各形态TOP3 - 按胜率排序：V型急跌 > 强势横盘 > 放量回调 > 深度回调
pattern_order = ['V型急跌', '强势横盘', '放量回调', '深度回调']

for pattern in pattern_order:
    df_pattern = df[df['pattern'] == pattern].head(3)
    if len(df_pattern) > 0:
        # 添加胜率标注
        win_rate = {'V型急跌': '97.2%', '强势横盘': '98.6%', '放量回调': '91.2%', '深度回调': '88.2%'}
        story.append(Paragraph(f'{pattern} TOP3（胜率{win_rate[pattern]}）', styles['CH1']))
...
if high > 0:
    story.append(Paragraph(f'• 高质量信号{high}只（矩子科技、银禧科技），建议优先关注', styles['CBullet']))
story.append(Paragraph(f'• V型急跌胜率最高（97.2%），信号数最多（6只）', styles['CBullet']))
story.append(Paragraph(f'• 结合BullScore评分和二波形态双重筛选，提升信号质量', styles['CBullet']))
