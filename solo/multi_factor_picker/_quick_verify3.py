...
print(f"\n5月: {len(may_signals)}个信号, {len(set(s['date'] for s in may_signals))}天有信号")
print(f"6月: {len(jun_signals)}个信号, {len(set(s['date'] for s in jun_signals))}天有信号")

# 6月日均
jun_days = len(set(s['date'] for s in jun_signals))
if jun_days > 0:
    print(f"\n6月统计（前200只合格股）:")
    print(f"  有信号天数: {jun_days}天（6月约22个交易日）")
    print(f"  6月日均: {len(jun_signals)/22:.2f}只（按22交易日算）")
    print(f"  推算全量946只日均: {len(jun_signals)/22 * 946/200:.2f}只")
    print(f"  推算全量有信号天数比例: {min(100, jun_days/22*100*946/200):.1f}%")
...
for d in sorted_dates[-5:]:
    ...
    print(f"  {d}:")
    for s in sorted(day_signals, key=lambda x: -x['score'])[:5]:
