# -*- coding: utf-8 -*-
"""从 entry_precision CSV 生成 PDF 报告（自动找最新CSV）"""
import os, glob
import pandas as pd
...
if not csv_files:
    print('错误：未找到CSV文件')
    exit(1)
csv_path = max(csv_files, key=os.path.getmtime)  # 最新的CSV
...
stats_data = [
    ['指标', '数值'],
    ['信号数', str(len(df))],
    ['+1d平均收益', f"{df['return_1d'].mean():.2f}%"],
    ['+1d胜率', f"{(df['return_1d']>0).sum()/len(df)*100:.0f}%"],
    ['+10d平均收益', f"{df['return_10d'].mean():.2f}%"],
    ['+10d胜率', f"{(df['return_10d']>0).sum()/len(df)*100:.0f}%"],
]
