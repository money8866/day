...
**今日第一梯队共1只**：**航宇科技（688239.SH）**B级

1. **航宇科技（688239.SH）[B级]**：ENTRY=86 EXPANSION=80 TAIL=81 FE=60｜供给吸收15/15、锁筹=是、RS20=95｜触发价51.49 止损48.43（约-5.9%） 目标58.38 建议仓位15%。
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 1 | B | 688239.SH | 航宇科技 | 86 | 80 | 81 | 60 | T1 | 51.49 | 48.43 | 58.38 | 15% | 锁筹+突破T1，扩张20/20 |
...
- ⑤ RS加速：RS5=97 RS10=94 RS20=95，加速度=+3（平稳）。
...
|---|---|---|---|---|---|---|---|
| 1 | 002602.SZ | 世纪华通 | 85 | 83 | T20_ROCKET_WATCH | 收盘位置 | 扩张空间20，供给吸收7 |
...
|---|---|---|---|---|---|---|---|---|---|
| 002724.SZ | 海洋王 | GOOD | 20260828 | 6.78 | 20260904 | 0.64 | -0.3% | -1.0% |
| 603605.SH | 珀莱雅 | GOOD | 20260902 | 62.30 | 20260903 | 0.67 | +0.3% | +0.4% |
...
|---|---|---|---|---|---|---|---|---|---|---|---|
| 002602.SZ | 世纪华通 | WAIT | PENDING | N | WEAK | NA | FAIL: 未完成缩量锁筹（5日量比1.00）；天量后观察0日<3日 | PASS | 68.0/7.0 | 15.73 | 关注能否收于TRIGGER 16.16上方 |
| 603444.SH | 吉比特 | WAIT | CONFIRMED | Y | NEUTRAL | NEAR | FAIL: 未完成缩量锁筹（5日量比0.69） | PASS | 70.8/4.2 | 399.90 | 等待缩量锁筹完成且天量后观察满3日 |
...
- **吉比特（603444.SH）** WAIT_REASON：SCORE=70.8<75（SCORE_GAP=4.2，需执行分≥75）；LOCK_GATE=FAIL: 未完成缩量锁筹（5日量比0.69）
  UPGRADE_CONDITION：收盘≥383.58 AND 当日量比≥1.20 AND Close>MA20向上 AND 突破质量≠WEAK AND 执行分≥75（GAP=4.2）AND 锁筹/观察门PASS
- **小商品城（600415.SH）** WAIT_REASON：五层Gate全过（无缺口）
  UPGRADE_CONDITION：收盘≥12.86 AND 当日量比≥1.20 AND Close>MA20向上 AND 突破质量≠WEAK AND 执行分≥75（GAP=0.0）AND 锁筹/观察门PASS
- **航宇科技（688239.SH）** WAIT_REASON：BREAKOUT_READY=FAIL（当日量比0.99<1.20（需≥1.20））；BREAKOUT_QUALITY=WEAK（需当日量比≥1.3、收盘位≥0.70且无长上影）；SCORE=63.9<75（SCORE_GAP=11.1，需执行分≥75）
...
- **药明康德（603259.SH）** STREAK=0 DP=PULLBACK_RE → R2 首入再入日：首日入池但今日为非首个回踩GOOD日（回测pf 0.76）
...
### 【T20_ROCKET_WATCH】世纪华通（002602.SZ）
...
- 板块：游戏（强度63） | 基本面：S（90） | 资金质量：50
- 交易计划：入场=0.00 止损=14.59 目标1=0.00 目标2=0.00 建议仓位=10%
- WAIT_REASON：等待缩量锁筹；天量后观察不足3日；等待突破T0_High
- 为什么它像中际旭创：同为阶段级天量(量比3.1)、天量日价格强(涨8.0%/收盘位0.81)、相似度77.6；差异：缩量不足(量缩比1.00 vs 案例0.48)，尚未锁筹，尚未二次突破

### 【T20_ROCKET_WATCH】吉比特（603444.SH）

- V3双评分：ENTRY=76.7 | EXPANSION=83.5 | TAIL=82.5(SAMPLE_LOW) | HVT分=0.0 | 分层=T3
- ENTRY分解：价格结构17.4 量能16.0 回踩11.0 RS11.0 板块6.3 盈亏比15.0
- EXPANSION分解：空间20.0 压缩12.5 动量13.0 RS加速8.0 量效8.0 吸收9.0 基本面10.0 催化3.0
- FE未来扩张：FE=66 FE10=74 FE20=72 FE60=91 FE120=86 | Lifecycle=EARLY TrendGain=26% Continuation=70 ExtRisk=6 Type=RE_ACCELERATION
- FE分解：accel=60 base_expansion=66 continuation=70 dd20=0 dd60=0 fundamental=85 lifecycle=90 ma=65 overhead=1 platform=35 rs=66 space=57
- 分级：收盘位置=A 放量=A RS5=79 RS10=68 RS20=67 RS加速度=+13
- 天量日 20260729：涨幅8.2% 收盘位0.89 换手5.8% | 天量后回撤-0.6% 量缩比0.69 锁筹=否
- 二次突破：20260731 放量1.6x 收盘位0.70 幅度2.5% 距T0=2日 假突破=否
- 突破回踩：判定NEAR 缩量比0.51 低点374.00@20260814 低点vsT0_High-1.5% 当前vs突破+2.7%
- 板块：游戏（强度63） | 基本面：S（90） | 资金质量：90
- 交易计划：入场=0.00 止损=348.90 目标1=0.00 目标2=0.00 建议仓位=10%
- WAIT_REASON：等待缩量锁筹；突破日特征未达T1/T2归因分层（放量倍数/突破幅度/距T0天数）
- 为什么它像海天精工：同为阶段级天量(量比1.9)、天量日价格强(涨8.2%/收盘位0.89)、相似度76.2；差异：尚未锁筹

### 【T20_ROCKET_WATCH】小商品城（600415.SH）

- V3双评分：ENTRY=80.7 | EXPANSION=81.5 | TAIL=81.4(SAMPLE_LOW) | HVT分=0.0 | 分层=T3
...
- 突破回踩：判定NEAR 缩量比0.57 低点12.52@20260903 低点vsT0_High-1.6% 当前vs突破+1.2%
- 板块：消费（强度63） | 基本面：A（78） | 资金质量：90
- 交易计划：入场=0.00 止损=11.68 目标1=0.00 目标2=0.00 建议仓位=15%
- WAIT_REASON：突破日特征未达T1/T2归因分层（放量倍数/突破幅度/距T0天数）；回撤过大
- 为什么它像中际旭创：同为阶段级天量(量比2.5)、天量日价格强(涨6.9%/收盘位0.92)、相似度82.3
...
- 突破回踩：判定NEAR 缩量比0.64 低点50.41@20260902 低点vsT0_High-1.1% 当前vs突破+5.6%
...
### 【T20_ROCKET_WATCH】中国人保（601319.SH）
...
- V3双评分：ENTRY=72.9 | EXPANSION=47.9 | TAIL=51.6(SAMPLE_LOW) | HVT分=0.0 | 分层=T3
- ENTRY分解：价格结构15.7 量能17.7 回踩11.5 RS11.7 板块6.3 盈亏比10.0
- EXPANSION分解：空间1.9 压缩9.5 动量11.0 RS加速8.0 量效1.0 吸收5.5 基本面8.0 催化3.0
- FE未来扩张：FE=80 FE10=95 FE20=91 FE60=89 FE120=80 | Lifecycle=EARLY TrendGain=31% Continuation=80 ExtRisk=4 Type=DISTRIBUTION_RISK
- FE分解：accel=60 base_expansion=76 continuation=80 dd20=0 dd60=0 fundamental=79 lifecycle=90 ma=100 overhead=0 platform=45 rs=75 space=85
- 分级：收盘位置=A+ 放量=D RS5=86 RS10=45 RS20=78 RS加速度=+8
- 天量日 20260820：涨幅6.5% 收盘位0.96 换手3.9% | 天量后回撤6.0% 量缩比0.52 锁筹=否
...
- 突破回踩：判定NEAR 缩量比1.14 低点6.80@20260904 低点vsT0_High-2.3% 当前vs突破-0.6%
