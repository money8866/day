...
def main():
    ...
    # 防御性/低波动板块（银行、证券、保险、红利）不适合"分歧低吸"短线策略
    DEFENSIVE_ETFS = {'银行', '证券', '保险', '红利'}
    ...
    for _, row in result_df.iterrows():
        if row['信号'] in ['主线启动', '第一次分歧低吸', '主升浪'] and portfolio_df[portfolio_df['ts_code'] == row['ETF']].empty:
            if row['行业'] in target_industries and row['总评分'] >= 60 and emotion_score > 50:
                # 防御性板块排除"分歧低吸"信号（只考虑主线启动和主升浪）
                if row['行业'] in DEFENSIVE_ETFS and row['信号'] == '第一次分歧低吸':
                    continue
                # 检查对应主题状态
