...
def analyze_score_detail(df, ts_code, name):
    ...
    print(f"  基础分合计                    = {score_before:.2f}")
    
    # 失败概率惩罚
    score_after = score_before - fail_prob * 8
    print(f"\n  减去失败概率惩罚 (fail_prob * 8): {score_before:.2f} - {fail_prob * 8:.2f} = {score_after:.2f}")
    
    # 放大评分
    final_score = score_after * 1.3
...
def main():
    ...
    for i, r in enumerate(results_sorted, 1):
        print(f"  {i}. {r['name']} ({r['code']}) - 评分: {r['score']:.2f}, 失败概率: {r['fail_prob']:.2%}")
