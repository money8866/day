...
def find_backbones_in_sector(stock_list, sector_name, sector_info):
    """在板块成分股中寻找中军"""
    backbones = []
...
def load_stock_concept_map():
    """加载股票概念映射"""
    stock_concept_path = os.path.join(BASE_DIR, "../cache_daily/stock_concept_map.pkl")
...
def get_sector_stocks(sector_name, sector_type, concept_stock_map):
    """获取板块成分股"""
    if sector_type == '主题':
        return concept_stock_map.get(sector_name, [])
    else:
        return []
...
def main():
    ...
    print(f"\n【第二步】对前{TOP_SECTOR_COUNT}个主线板块进行中军分析...")
    
    for _, sector in top_sectors.iterrows():
        sector_name = sector['主线']
        sector_type = sector['类型']
        
        stocks = get_sector_stocks(sector_name, sector_type, concept_stock_map)
        
        if not stocks or len(stocks) < 10:
            print(f"\n跳过 {sector_name}：成分股不足")
            continue
        ...
        backbones = find_backbones_in_sector(stocks, sector_name, sector_info)
        
        if backbones:
            print(f"\n找到 {len(backbones)} 只潜在中军:")
            for bb in backbones[:3]:
                print(f"  - {bb['name']}({bb['ts_code']}) | 评分: {bb['中军评分']} | 市值: {bb['市值(亿)']}亿")
