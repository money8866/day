...
def generate_signals(df):
    """修改版信号逻辑：
    买入：多头排列 + 动能>0 + 价格回撤到MA20附近（< MA20*1.03）+ 放量
    卖出：仅ATR动态止损（去掉动能转负卖出）
    """
    position = 0
    signals = []
    ...
    for idx, row in df.iterrows():
        ...
        if pd.isna(current_mom) or pd.isna(current_stop) or pd.isna(ma20) or pd.isna(ma60):
        ...
        if position == 0:
            ...
            # 1. 动能>0（趋势向上）
            # 2. MA10 > MA60（中期多头）
            # 3. 价格回撤到MA20附近（< MA20*1.03，不追高）
            # 4. 成交量不萎缩（> vol_ma5 * 0.8）
            mom_ok = current_mom > 0
            trend_ok = ma10 > ma60
            pullback_ok = current_close < ma20 * 1.03
            vol_ok = pd.isna(vol_ma5) or vol > vol_ma5 * 0.8

            if mom_ok and trend_ok and pullback_ok and vol_ok:
                position = 1

        elif position == 1:
            # 卖出条件：仅ATR动态止损（去掉动能转负）
...
def main():
    ...
    if len(result["trades"]) > 0:
        ...
        # 保存
        output_file = r"C:\Users\kongx\mystock\etf_backtest_v2_result.csv"
