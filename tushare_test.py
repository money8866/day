...
#
    ...
    cond_xh2 = C.iloc[-1] / C.iloc[-2]>0.99  and  C.iloc[-1] / ma5.iloc[-1] <1.08 and C.iloc[-1] / ma5.iloc[-1] > 0.99
    ...
    if emotion_stage == "强":
        XH = (cond_xh1 and cond_xh2) or (cond_xh3 and cond_xh4)
    else:
        XH = cond_xh3 and cond_xh4
...
3、个股分析:输出分析后筛选出的股票列表(只要爆发力最强和近期主线关联最紧密的两个,考虑主线轮动因素)，要求理由清晰且有数据支撑，并给出买卖点/未来上涨空间预估
...
内容(分成以下部分)：
1、大盘情绪(含涨跌停数等几个数据指标)和仓位建议
2、主线板块分析(加粗显示主线龙头和成交量最大趋势最强的中军，并分析主线板块的阶段和持续性，给出数据支撑和逻辑理由）
3、个股分析:输出分析后筛选出的股票列表(只要爆发力最强和近期主线关联最紧密的两个,考虑主线轮动因素)，要求理由清晰且有数据支撑，并给出买卖点/未来上涨空间预估
