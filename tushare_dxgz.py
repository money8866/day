...
#
    ...
    cond_xh2 = C.iloc[-1] / C.iloc[-2]<1  and  C.iloc[-1] / ma5.iloc[-1] <1.02 and df['low'].iloc[-1] / ma5.iloc[-1] <1
...
1综合评分较高和属于近期最强和反复活跃的主线板块的个股，结合AI新闻面和市场情绪分析,优选评分高/超短线情绪强的个股,适合低吸埋伏
...
输出内容：
标题：每日复盘低吸跟踪版({TRADE_DATE})
内容(分成以下部分)：
1、个股分析:输出分析后筛选出的股票列表(只要爆发力最强和近期主线关联最紧密的两个,考虑主线轮动因素)，要求理由清晰且有数据支撑，并给出买卖点/未来上涨空间预估
2、附上属于主线板块的个股列表(严格按今日量化候选股票池输出,按个股综合评分从高到低排序,显示序号)，并给出每只股票的综合评分和相关主线，供读者参考
...
"""
    ...
    with open(
        os.path.join(
            REPORT_DIR,
            f"Deepseek_Dxgz_{TRADE_DATE}.md"
        ),
        "w",
        encoding="utf-8"
    ) as f:
