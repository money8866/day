...
def mcp_call(tool, **kwargs):
    ...
    ps1 = os.path.join(os.environ.get("TEMP", "C:\\temp"), "tdx_debug6.ps1")
...
resp = mcp_call("wenda_notice_query", query=f"龙蟠科技|{start.strftime('%Y%m%d')}|{end.strftime('%Y%m%d')}|", pageSize="50")
data = resp.get("data", [])
print(f"总条目数: {len(data)-1}")
for i, item in enumerate(data[1:21], 1):
    if not isinstance(item, list) or len(item) < 5:
        continue
    title = (item[0] or "")[:60]
    content = (item[4] or "")[:100]
    codes = re.findall(r"证券代码[：:]\s*(\d{6})", content)
    in_title = "龙蟠科技" in title
    print(f"  {i}. codes={codes} in_title={in_title} | title={title}")
