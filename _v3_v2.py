# -*- coding: utf-8 -*-
"""V3 修正版 - PE从daily_basic取，市值直接用total_mv"""
...
def main(date_str):
    print(f"V3 修正版 - {date_str}")
    print("=" * 60)

    # Step1: 申万动量（可选，跳过不影响选股）
    hot = []
    try:
    ...
    for _, row in df_basic.iterrows():
        ...
        if fsc >= 15:
            scored.append({
                'ts_code': tc, 'price': row['close'],
                'pe': row['pe'], 'mv_yi': row['mv_yi'],
                'fin_score': fsc
            })

    print(f"    基本面≥20分: {len(scored)} 只")
    if not scored:
        print("无候选"); return

    # Step4: K线技术评分
    print(f"[Step4] K线技术分析...")
    results = []
    for i, s in enumerate(scored):
        ...
        results.append({
            ...
            'tech_score': tsc, 'fin_score': s['fin_score'],
            'repeat': repeat, 'total': tsc + s['fin_score'],
            'grade': grade, 'ma5': ma5_v
        })

        if (i+1) % 20 == 0:
            print(f"    K线分析 {i+1}/{len(scored)} ...")
